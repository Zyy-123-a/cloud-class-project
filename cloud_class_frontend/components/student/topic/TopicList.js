...
export default function TopicList({
    ...
    const columns = [
        ...(showCheckbox ? [{
                ...
                <Checkbox
                    ...
                    onChange={(e) => {
                        if (onSelectChange) {
                            if (e.target.checked) {
                                if (!isTeacher) {
                                    const myTopics = topics.filter(t => t.uid === currentUserId);
                                    onSelectChange(myTopics.map(t => t.tid));
                                } else {
            ...
            width: 50,
            render: (_, record) => {
                const canSelect = isTeacher || record.uid === currentUserId;
                return (
                    <Checkbox
                        ...
                        disabled={!canSelect || batchLoading}
                    />
                );
            }
        }] : []),
            ...
            render: (text, record) => (
                <Space>
                    ...
                    <span>
                        ...
                        {!isTeacher && record.uid === currentUserId && (
                            <Tag color="blue" style={{marginLeft: 4}}>我的</Tag>
                        )}
                    </span>
                </Space>
            )
    ...
    return (
        <Table
            ...
            rowClassName={(record) => {
                if (!isTeacher && record.uid === currentUserId) {
                    return 'my-topic-row';
                }
                return '';
            }}
